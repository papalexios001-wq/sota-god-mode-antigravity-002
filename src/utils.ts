...
export const stripMarkdownCodeBlocks = (text: string): string => {
  ...
  const codeBlockStartRegex = /^```(?:json|JSON|javascript|Javascript|JS|js|html|HTML)?\\s*/;
...
export const parseJsonWithAiRepair = async (
  ...
  throw new Error(
    ...
    `(Total length: ${responseText.length} chars)`
  );
};
...
print("Utils file generated successfully!")
print(f"Total characters: {len(utils_ts)}")
